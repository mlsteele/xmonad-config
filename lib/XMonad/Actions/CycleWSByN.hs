-- CycleByN Module
-- Allows switching workspaces by a delta relative to the current.
-- The implementation is just CycleWS, but exposing it's switchWorkspace function.
...
module XMonad.Actions.CycleWSByN (cycleByN) where
